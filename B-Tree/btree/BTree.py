...
class BTree:
    ...
    def demote(self, page):
        ...
        After these methods are done with their jobs, this method moves forward to a set of verifications: if, after this procedure, the parent_page violates the B-Tree minimum degree rule, then BTree.demote() is called with parent_page as a parameter. Otherwise, if parent_page actually is the root of the BTree object and its length is 0, it means that the depth of the B-Tree has lowered, and the self.root attribute must be resetted. This is done by calling BTree.recreate_root().
    ...
    def __repr__(self):
        ...
        This method takes advantage of BTree.get_depth() and BTree.get_pages_of_depth(). It obtains all pages of each depth and does some calculations to figure out how much space is to be added between the elements of each level. It stores the final result of each level as a string under representations, which at the end is merged (with each line separated by a line break) and returned.
        """
        depth = self.get_depth()
        representations = [
            " ".join([str(item) for item in self.get_pages_of_depth(depth)])
        ]
        max_length = len(representations[0])

        for d in range(depth - 1, -1, -1):
            list_of_elements = self.get_pages_of_depth(d)
            string_representation = " ".join(
                [str(item) for item in self.get_pages_of_depth(d)]
            )
            ...
            representation = (
                spaces
                + spaces.join([str(item) for item in self.get_pages_of_depth(d)])
                + spaces
            )
    ...
    def get_depth(self):
        """Returns the depth of the BTree object.
        ...
        int
            ...
            The depth of the BTree object.

        Methodology
        -----------

        This method just calls BTree.get_pages_of_depth() with 1 as a parameter, and increments the value by one unit until it returns None, taking note of the last number. At the end, it returns the depth obtained.
        """
        depth = 0
        while self.get_pages_of_depth(depth + 1):
            depth += 1
        return depth

    def get_pages_of_depth(self, depth, merge_lists=True):
        """Returns all pages in the same, given depth.
        ...
        list, None
            ...
            A list of lists containing the element (affected by the merge_lists parameter) or None, in case the depth provided resulted in an empty list.
        """
        try:
            pages = self.root.get_pages_of_depth(depth)
            if merge_lists:
